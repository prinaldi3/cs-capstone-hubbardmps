...
function phi_tl(time, params, independent)
    if independent
        ...
        return (params.a * params.strength / params.field) * (sin(params.field * time / (2*params.cycles))^2) * sin(params.field * time)
    end
end
...
function propogation(ground::MPS, params, tf, method, dti, epsilon, independent, cutoff)
    ...
    f = getfield(Main, Symbol(method))

    # run for the entire time interval
    while time < tf

        # get the MPS at time + dt
        next_psi = f(psi, dt, time, params, independent, cutoff)
        ...
        energy = inner(psi, get_ham(time, params, independent), psi)
        @show time
        @show energy
        println()
...
function get_current(time, params, independent)
    ...
    left = -1im * params.a * eiphiconj
    right = 1im * params.a * eiphi
